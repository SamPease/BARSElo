#!/usr/bin/env python3
"""
Optuna wrapper tuned specifically for the `calculate_trueskill_mov.py` defaults.
This computes search bounds as a percentage of the defaults defined in
`calculate_trueskill_mov.py` and runs Optuna minimizing average log-loss.
Supports multiple simulation runs per trial via `sim_count`.
"""
import argparse
import json
import optuna
...
from calculate_trueskill_mov import INITIAL_MU, INITIAL_SIGMA, INITIAL_BETA, INITIAL_TAU, DRAW_PROBABILITY


def objective_factory(mu, teams_file, games_file, max_games, seed,
                      sigma_min, sigma_max, beta_min, beta_max,
                      tau_min, tau_max, draw_min, draw_max, sim_count=5):
    def objective(trial):
        ...
        avg_logloss, count = evaluate_params_mov(mu, sigma, beta, teams_file, games_file,
                                                max_games=max_games, sim_count=sim_count, seed=seed,
                                                tau=tau, draw_probability=draw)
        if count == 0:
            return float('inf')
...
def main():
    parser = argparse.ArgumentParser(description='Optuna tuner for calculate_trueskill_mov defaults')
    ...
    parser.add_argument('--sim-count', type=int, default=5, help='Number of randomized simulations per trial')
    parser.add_argument('--max-games', type=int, default=0)
    parser.add_argument('--out-csv', default='analysis/trueskill_mov_optuna_trials.csv')
    parser.add_argument('--best-out', default='analysis/best_trueskill_mov_optuna.json')
    ...
    objective = objective_factory(args.mu, args.teams, args.games, max_games, args.seed,
                                 sigma_min, sigma_max, beta_min, beta_max,
                                 tau_min, tau_max, draw_min, draw_max, args.sim_count)

    print(f"Running Optuna: {args.trials} trials (n_jobs={args.n_jobs}) sim_count={args.sim_count}")
